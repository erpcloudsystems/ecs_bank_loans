...
def check_instance(instance_date):
		if  isinstance(instance_date, str):
			return datetime.datetime.strptime(instance_date, "%Y-%m-%d")
		return datetime.datetime.strptime(instance_date.strftime("%Y-%m-%d"), "%Y-%m-%d")
class BankLoan(Document):
	...
	def calculate_repayment_and_interest_amount(self):
		if self.calculate_repayment_schedule_automatically and not self.calculate_rate_of_interest:
			...
			self.repayment_amount = -1 * npf.pmt(0.01 * self.rate_of_interest / (360 / self.repayment_every), self.no_of_repayments, self.loan_amount)
			self.interest_amount = 0.01 * self.loan_amount * self.rate_of_interest / (360 / self.repayment_every)
			if self.type_of_interest =="Fixed":
				rate_of_interest = self.rate_of_interest * (self.no_of_repayments / 12)

				self.interest_amount =0.01* self.loan_amount * rate_of_interest / (self.no_of_repayments)
				self.repayment_amount = (self.loan_amount / self.no_of_repayments) + self.interest_amount
		if self.calculate_repayment_schedule_automatically and self.calculate_rate_of_interest:
			import numpy_financial as npf
			self.rate_of_interest = npf.rate(self.no_of_repayments, (-1 * self.repayment_amount), self.loan_amount, 12) * 12 * 100
			self.interest_amount = 0.01 * self.loan_amount * self.rate_of_interest / (360 / self.repayment_every)
	...
	def calculate_repayment_schedule(self):
		if self.calculate_repayment_schedule_automatically:
			...
			start_day = check_instance(payment_date).day
			...
			while (a > 0):
				...
				next_payment_date= check_instance(next_payment_date)
				next_payment_date.replace(day =start_day ) 
				next_interest_amount = interest_amount
				if self.type_of_interest != "Fixed":
